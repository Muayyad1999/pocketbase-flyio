...
chmod 755 "$POCKETBASE_DATA_DIR"

# Fix permissions if database exists but is read-only
if [ -f "$POCKETBASE_DATA_DIR/data.db" ]; then
  chmod 644 "$POCKETBASE_DATA_DIR/data.db" 2>/dev/null || true
fi
